    ...
    <div className="page-wrap">
      <div className={styles.pageHead}>
        <div>
          ...
          <p style={{ color: "var(--subtle)", fontSize: 14 }}>Track every property from saved to applied.</p>
        </div>
        <button className="btn-primary" onClick={() => setShowAdd(!showAdd)}>+ Add property</button>
      </div>

      {showAdd && (
        <div className="panel fade-in" style={{ marginBottom: "1.5rem", display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
          <div className="field" style={{ flex: 2, marginBottom: 0 }}>
            <label>Address</label>
            <input type="text" placeholder="e.g. 123 Main St, Syracuse NY"
          ...
          <button className="btn-primary" onClick={addProperty}>Add</button>
        ...
        {STAGES.map((col) => {
          ...
          return (
            <div key={col.id} className={styles.col}>
              ...
              {cards.length === 0 && <div className={styles.emptyCol}>No properties here yet</div>}
              {cards.map((p) => (
                <div key={p.id} className={`${styles.card} ${p.topPick ? styles.topPick : ""}`}>
                  ...
                  <div className={styles.moveRow}>
                    {STAGES.filter((s) => s.id !== col.id).map((s) => (
                      <button key={s.id} className={styles.moveBtn} onClick={() => move(p.id, s.id)} title={`Move to ${s.label}`}>
                        → {s.label}
                      </button>
                    ))}
                  </div>
