...
const REALTORS: Realtor[] = [
  {
    ...
    reviewer: "Sarah T., bought in DeWitt 2024",
    verified: true,
  },
  {
    ...
    review: "She walked me through every document at closing. I never felt rushed or confused. She explained every fee before we got to the table.",
    reviewer: "James K., bought in Camillus 2024",
    verified: false,
  },
  {
    ...
    review: "David negotiated $4k off my rent for the first year and was upfront about every fee from the first call. No surprises.",
    reviewer: "Monica R., renting in Syracuse 2025",
    verified: false,
  },
];
  ...
  return (
    <div className="page-wrap">
      <div className="page-hd">
        ...
        <p>
          Vetted agents with verified reviews, transparent fee disclosure, and no
          hidden charges.
        </p>
      </div>
      ...
      <div className={styles.redFlags}>
        ...
        <button
          ...
          onClick={() =>
            onAskAi(
              "What are the biggest realtor red flags I should watch out for when buying a home? What are my rights if I encounter them?"
            )
          }
        >
      ...
      <p className="section-label" style={{ marginBottom: 14 }}>
        Verified realtors near Syracuse, NY
      </p>
      ...
      <div className={styles.grid}>
        {REALTORS.map((r) => (
          <div
            key={r.name}
            className={`${styles.card} ${r.verified ? styles.cardTop : ""}`}
          >
            {r.verified && (
              <span className={styles.topBadge}>✓ NestWise Verified</span>
            )}
            ...
            <div className={styles.stats}>
              {[
                { val: r.rating,     label: "Rating"     },
                { val: r.deals,      label: "Deals"      },
                { val: r.complaints, label: "Complaints" },
              ].map(({ val, label }) => (
            ...
            {r.badges.map((b) => (
              <div
                key={b.text}
                className={`trust-badge ${b.type === "green" ? "tb-green" : "tb-blue"}`}
              >
                {b.text}
              </div>
            ))}
            ...
            <button
              ...
              onClick={() =>
                onAskAi(
                  `I'm considering working with a realtor named ${r.name} who specializes in ${r.title}. What questions should I ask them before signing anything?`
                )
              }
            >
