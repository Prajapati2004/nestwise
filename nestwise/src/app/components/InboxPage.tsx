...
interface Thread {
  title: string; sub: string; preview: string; time: string;
  tag: "pending" | "replied" | "tour" | "declined"; unread: boolean; msgs: Message[];
}

const THREADS: Thread[] = [
  {
    title: "88 Westcott St", sub: "$1,650/mo · 2 bd · 1 ba", preview: "Hi! Yes still available...",
    time: "Today 9:41am", tag: "replied", unread: true,
    msgs: [
  ...
  {
    title: "312 Oak Blvd", sub: "$229,000 · 3 bd · 1 ba", preview: "Please fill out our application...",
    time: "Today 8:15am", tag: "tour", unread: true,
    msgs: [
  ...
  {
    title: "201 Park St", sub: "$1,400/mo · 1 bd · 1 ba", preview: "Unfortunately this unit was just...",
    time: "Yesterday", tag: "declined", unread: true,
    msgs: [
  ...
  {
    title: "57 Elm St", sub: "$315,000 · 4 bd · 2.5 ba", preview: "Sent: Is price negotiable?",
    time: "Mon", tag: "pending", unread: false,
    msgs: [{ me: true, text: "Hi, I'm interested in this listing. Is the price negotiable?", time: "Mon 2pm" }],
  },
  {
    title: "142 Maple Ave", sub: "$289,000 · 3 bd · 2 ba", preview: "Sent: When was roof replaced?",
    time: "Sun", tag: "pending", unread: false,
    msgs: [{ me: true, text: "Is the price negotiable? Also when was the roof last replaced?", time: "Sun 11am" }],
  },
];

const TAG_LABELS: Record<Thread["tag"], string> = { pending: "No reply", replied: "Replied", tour: "Tour scheduled", declined: "Not available" };
const TAG_CLASSES: Record<Thread["tag"], string> = { pending: "stag stag-pending", replied: "stag stag-replied", tour: "stag stag-tour", declined: "stag stag-declined" };
...
export default function InboxPage() {
  ...
  function select(i: number) {
    setSelected(i);
    setThreads((prev) => prev.map((t, idx) => (idx === i ? { ...t, unread: false } : t)));
  }
  ...
  function send() {
    ...
    setThreads((prev) => prev.map((t, i) =>
      i === selected
        ? { ...t, msgs: [...t.msgs, { me: true, text, time: "Now" }], preview: `Sent: ${text.slice(0, 40)}`, tag: "pending" as const }
        : t
    ));
    setDraft("");
  }
        ...
        <div className={styles.tpanel}>
          ...
          <div className={styles.tbody}>
            {active.msgs.length === 0
              ...
              : active.msgs.map((m, i) => (
                  <div key={i} className={`${styles.msg} ${m.me ? styles.me : styles.them}`}>
